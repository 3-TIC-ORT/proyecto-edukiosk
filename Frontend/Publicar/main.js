...
document.addEventListener('DOMContentLoaded', () => {
  ...
  form.addEventListener('submit', async (e) => {
    ...
    console.log('✅ Publicación lista para enviar:', publicacion);
    ...
    try {
      ...
      if (res.ok) {
        mensajePopUp("✅ Publicación enviada correctamente", "#28a745");
        form.reset();
        actualizarCamposSegunRecurso();
        actualizarEspecialidadesSegunGrado();
      } else {
        mensajePopUp("❌ Error al enviar la publicación", "#e92828ff");
      }
    } catch (error) {
      console.error("Error al conectar con el backend:", error);
      mensajePopUp("⚠️ Error de conexión con el servidor", "#e92828ff");
    }
  });
});
